  ...
  return (
    <div className="group rounded-2xl overflow-hidden bg-white border border-white80 shadow-sm hover:shadow-xl transition-all duration-300">
      <div className="relative h-64 w-full overflow-hidden bg-white80">
        ...
        <div className="absolute inset-0 flex items-center justify-center text-black60 font-semibold text-lg">
          {/* Placeholder for real image */}
          {title} Image
        </div>
      </div>
      <div className="p-6">
        <span className="text-sm font-semibold text-purple mb-2 block tracking-wider uppercase">{category}</span>
        <h3 className="text-xl font-bold text-black mb-4 group-hover:text-skyBlue transition-colors">{title}</h3>
        {href && (
          <Link href={href} className="inline-flex items-center text-black font-medium hover:text-purple transition-colors">
